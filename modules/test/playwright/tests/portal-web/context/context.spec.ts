...
test(
	...
	async ({page}) => {
		...
		await test.step(`Portal's web context name is referenced in import maps`, async () =>
			testImportMapsPattern(page, pattern));

		await test.step(`Portal's web context name is referenced in <script> tags`, async () =>
