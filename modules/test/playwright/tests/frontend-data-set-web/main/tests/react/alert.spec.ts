...
test('Alert component', {tag: ['@LPD-75254']}, async ({page}) => {
	...
	await test.step('Check that the "Reload" button request new data and closes the alert', async () => {
		...
		const ascendingIDCells = [cells[1], cells[10], cells[19], cells[28]];
		...
		const descendingIDCells = [cells[1], cells[10], cells[19], cells[28]];
