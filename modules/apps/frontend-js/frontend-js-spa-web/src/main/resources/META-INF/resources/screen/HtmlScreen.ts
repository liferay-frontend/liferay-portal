...
class HtmlScreen extends RequestScreen {
	 ...
	 */
	allocateVirtualDocumentForContent(htmlString: string) {
	 ...
	 * same dom order.
	 */
	appendStyleIntoDocument_(newStyle: HTMLStyleElement) {
	 ...
	 */
	copyNodeAttributesFromContent_(content: string, node: HTMLElement) {
		content = content.replace(/[<]\s*html/gi, '<senna');
		content = content.replace(/\/html\s*>/gi, '/senna>');

		node.innerHTML = content;

		const placeholder: Nullable<HTMLElement> = node.querySelector('senna');
