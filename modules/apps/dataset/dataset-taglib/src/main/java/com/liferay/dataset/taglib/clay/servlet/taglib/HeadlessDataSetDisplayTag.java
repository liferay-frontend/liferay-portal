...
import com.liferay.dataset.taglib.clay.data.set.filter.ClayDataSetFilterSerializer;
import com.liferay.dataset.taglib.clay.data.set.model.ClayPaginationEntry;
import com.liferay.dataset.taglib.clay.data.set.servlet.taglib.util.ClayDataSetActionDropdownItem;
...
public class HeadlessDataSetDisplayTag extends IncludeTag {
	...
	protected void setAttributes(HttpServletRequest httpServletRequest) {
		request.setAttribute(
			"dataset:headless-display:actionParameterName",
			_actionParameterName);
		request.setAttribute(
			"dataset:headless-display:activeViewSettingsJSON",
			_activeViewSettingsJSON);
		request.setAttribute("dataset:headless-display:apiURL", _apiURL);
		request.setAttribute("dataset:headless-display:appURL", _appURL);
		request.setAttribute(
			"dataset:headless-display:bulkActionDropdownItems",
			_bulkActionDropdownItems);
		request.setAttribute(
			"dataset:headless-display:clayDataSetActionDropdownItems",
			_clayDataSetActionDropdownItems);
		request.setAttribute(
			"dataset:headless-display:clayDataSetDisplayViewsContext",
			_clayDataSetDisplayViewsContext);
		request.setAttribute(
			"dataset:headless-display:clayDataSetFiltersContext",
			_clayDataSetFiltersContext);
		request.setAttribute(
			"dataset:headless-display:clayPaginationEntries",
			_clayPaginationEntries);
		request.setAttribute(
			"dataset:headless-display:creationMenu", _creationMenu);
		request.setAttribute("dataset:headless-display:formId", _formId);
		request.setAttribute("dataset:headless-display:id", _id);
		request.setAttribute(
			"dataset:headless-display:itemsPerPage", _itemsPerPage);
		request.setAttribute("dataset:headless-display:module", _module);
		request.setAttribute(
			"dataset:headless-display:namespace", _namespace);
		request.setAttribute(
			"dataset:headless-display:nestedItemsKey", _nestedItemsKey);
		request.setAttribute(
			"dataset:headless-display:nestedItemsReferenceKey",
			_nestedItemsReferenceKey);
		request.setAttribute(
			"dataset:headless-display:pageNumber", _pageNumber);
		request.setAttribute(
			"dataset:headless-display:paginationSelectedEntry",
			_paginationSelectedEntry);
		request.setAttribute(
			"dataset:headless-display:portletURL", _portletURL);
		request.setAttribute(
			"dataset:headless-display:selectedItems", _selectedItems);
		request.setAttribute(
			"dataset:headless-display:selectedItemsKey",
			_selectedItemsKey);
		request.setAttribute(
			"dataset:headless-display:selectionType", _selectionType);
		request.setAttribute(
			"dataset:headless-display:showManagementBar",
			_showManagementBar);
		request.setAttribute(
			"dataset:headless-display:showPagination", _showPagination);
		request.setAttribute(
			"dataset:headless-display:showSearch", _showSearch);
		request.setAttribute(
			"dataset:headless-display:sortItemList", _sortItemList);
		request.setAttribute("dataset:headless-display:style", _style);
	}
