...
public class GetContentDashboardItemInfoMVCResourceCommand
		...
		try {
			...
			JSONObject jsonObject = contentDashboardItemFactoryOptional.flatMap(
				...
				contentDashboardItem -> JSONUtil.put(
					"categories",
					_getAssetCategories(contentDashboardItem, locale)
				).put(
					"className", contentDashboardItem.getClassName()
				).put(
					"classPK", contentDashboardItem.getClassPK()
				).put(
					"data", _getDataJSONArray(contentDashboardItem, locale)
				).put(
					"modifiedDate",
					_toString(contentDashboardItem.getModifiedDate())
				).put(
					"subType",
					_getSubtype(contentDashboardItem, locale)
				).put(
	...
	private JSONArray _getAssetCategories(
		ContentDashboardItem contentDashboardItem, Locale locale) {
	...
	private JSONArray _getDataJSONArray(
		ContentDashboardItem contentDashboardItem, Locale locale) {
		...
		return JSONUtil.putAll(
			stream.map(
				entry -> JSONUtil.put(
					...
					"languageId",
					LanguageUtil.getBCP47LanguageId(entry.getKey())
				).put(
