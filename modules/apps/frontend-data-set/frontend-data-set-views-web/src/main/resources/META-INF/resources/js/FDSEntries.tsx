...
type FDSEntryType = {
	...
	id: string;
	label: string;
	restApplicationLabel: string;
	restApplicationPath: string;
	restApplicationVersion: string;
	restSchema: string;
};

type RESTApplicationType = {
	label: string;
	path: string;
	version: string;
};
...
const RESTApplicationItem = ({
	...
	query: string;
	restApplication: RESTApplicationType;
}) => {
	const fuzzyLabelMatch = fuzzy.match(
		query,
		restApplication.label,
		FUZZY_OPTIONS
	);

	const fuzzyPathMatch = fuzzy.match(
		query,
		restApplication.path,
		FUZZY_OPTIONS
	);

	return (
		<ClayLayout.ContentRow className="rest-application">
			<span>
				{fuzzyPathMatch ? (
					<span
						dangerouslySetInnerHTML={{
							__html: fuzzyPathMatch.rendered,
						}}
					/>
				) : (
					<span>{restApplication.path}</span>
				)}

				{restApplication.version ? `/${restApplication.version}` : ''}
			</span>
			...
			<span className="context">
				{fuzzyLabelMatch ? (
					<span
						dangerouslySetInnerHTML={{
							__html: fuzzyLabelMatch.rendered,
						}}
					/>
				) : (
					<span>{restApplication.label}</span>
				)}
...
const RestApplicationDropdownMenu = ({
	...
	const onSearch = (query: string) => {
		...
		setRESTApplications(
			query
				? initialRESTApplications.filter(
						({label, path}: RESTApplicationType) => {
							return label.match(regexp) || path.match(regexp);
						}
				  ) || []
				: initialRESTApplications
		);
...
const AddFDSEntryModalContent = ({
	...
	const addFDSEntry = async () => {
		...
		const {label, path, version} = selectedRESTApplication;

		const body = {
			label: fdsEntryLabelRef.current?.value,
			restApplicationLabel: label,
			restApplicationPath: path,
			restApplicationVersion: version,
			restEndpointPath: selectedRESTEndpoint,
	...
	const getRESTSchemas = async (restApplication: RESTApplicationType) => {
		if (!restApplication) {
			return;
		}

		const {path, version} = restApplication;

		const versionPath = version ? `/${version}` : '';

		const response = await fetch(`/o${path}${versionPath}/openapi.json`);
	...
	const RestApplicationDropdown = () => (
		...
		>
			<RestApplicationDropdownMenu
				onItemClick={(item: RESTApplicationType) => {
	...
	const views = [
		{
			...
			schema: {
				fields: [
					...
					{
						contentRenderer: 'restApplication',
						fieldName: 'restApplicationLabel',
	...
	return (
		<div className="fds-entries">
			<FrontendDataSet
				...
				customDataRenderers={{
					restApplication: RestApplicationRenderer,
