...
public class FDSAdminFragmentRenderer implements FragmentRenderer {
				...
				JSONUtil.put(
					...
					JSONUtil.putAll(
						JSONUtil.put(
							...
							"typeOptions", JSONUtil.put("itemType", "DataSet")
						))))
		).toString();
	}
	...
	private String _buildFragmentHTML(
		...
		String objectEntryERC = dataSetObjectEntry.getExternalReferenceCode();
		...
		_reactRenderer.renderReact(
			...
			HashMapBuilder.<String, Object>put(
				...
				"id",
				"FDS_ADMIN_" + fragmentRendererContext.getFragmentElementId()
			).put(
				"itemsActions",
				...
				_getViewsJSONArray(
					...
					dataSetCardsSectionObjectEntries,
					String.valueOf(
						dataSetObjectEntryProperties.get(
							"defaultVisualizationMode")),
					dataSetTableSectionObjectEntries,
					dataSetListSectionObjectEntries, httpServletRequest)
			).build(),
	...
	private String _getAPIURL(
		...
		return _interpolateURL(
			_getNestedFields(sb.toString(), dataSetTableSectionObjectEntries),
			httpServletRequest);
	}
	...
	private JSONObject _getCreationMenuJSONObject(
		...
		Set<ObjectEntry> objectEntries = _getSortedRelatedObjectEntries(
			dataSetObjectDefinition, dataSetObjectEntry, "creationActionsOrder",
		...
		return JSONUtil.toJSONArray(
			...
			(ObjectEntry objectEntry) -> {
				...
				if (Objects.equals(type, "date") ||
					...
					JSONObject fromDateJSONObject = _getDateJSONObject(
						String.valueOf(properties.get("fromDate")));
					JSONObject toDateJSONObject = _getDateJSONObject(
						String.valueOf(properties.get("toDate")));

					boolean hasPreloadedData =
						(fromDateJSONObject != null) ||
						(toDateJSONObject != null);
					...
					return JSONUtil.put(
						...
						() -> {
							...
							return JSONUtil.put(
								"fromDate", fromDateJSONObject
							).put(
								"toDate", toDateJSONObject
							);
						}
					).put(
						"type", "dateRange"
	...
	private String _getNestedFields(
		...
		for (ObjectEntry tableSectionObjectEntry :
				dataSetTableSectionObjectEntries) {

			Map<String, Object> properties =
				tableSectionObjectEntry.getProperties();
		...
		return JSONUtil.toJSONArray(
			...
			(ObjectEntry objectEntry) -> {
				...
				if (FeatureFlagManagerUtil.isEnabled("LPD-19465")) {
					return JSONUtil.put(
						"active", properties.get("defaultSort")
					).put(
						"default", properties.get("defaultSort")
					).put(
	...
	private String _getValue(
		String defaultKey, String fallbackKey, Map<String, Object> properties) {

		String value = String.valueOf(properties.get(defaultKey));

		if (Validator.isNotNull(value)) {
			return value;
		}

		return String.valueOf(properties.get(fallbackKey));
	}
